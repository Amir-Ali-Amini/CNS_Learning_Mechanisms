...
def plot(
    ...
    if print_activities:
        ...
        figsize=(12 * scaling_factor, (7 if len(parameters) else 6) * scaling_factor),
    )
    else :
        fig, axd = plt.subplot_mosaic(
            (
        ...
        # "image" will contain a square image. We fine-tune the width so that
        # there is no excess horizontal or vertical margin around the image.
        figsize=(12 * scaling_factor, (5 if len(parameters) else 4) * scaling_factor),
    )
    ...
    axd["B"].scatter(
        ...
        vmin=0,
        vmax=ngs[1].network.number_of_inputs,
        label=f"{ngs[0].tag}",
    )
    ...
    axd["C"].scatter(
        ...
        s=2000 // ngs[1].network.number_of_inputs,
        vmin=0,
        vmax=ngs[1].network.number_of_inputs,
        label=f"{ngs[1].tag}",
    )
    ...
    for step in range(
        0,
        ngs[0].network.iteration,
        ngs[0].network.input_period * ngs[1].network.number_of_inputs,
    ):
